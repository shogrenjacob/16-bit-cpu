...
This will open a large file of hex values, but at the bottom there is an open button to select a pre-defined file to run. I have made some tests and a program named _fib.txt_ that calculates the 8th value in the [Fibonacci Sequence](https://en.wikipedia.org/wiki/Fibonacci_sequence). You can also create your own programs using the Opcodes from the [Instruction Set](instruction-set.md), and I plan to expand this project by creating an assembler for easier programming.
